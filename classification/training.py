...
class ClassificationTrainingApp:
    ...
    def init_model(self):
        ...
        if self.cli_args.finetune:
            ...
            model.load_state_dict(
                {
                    k: v for k,v in data['model_state'].items()
                    if k.split('.')[0] not in blocks[-1]
                },
                strict=False,
            )
    ...
    def do_training(self, epoch_index, train_dl):
        ...
        training_metrics_g = torch.zeros(
            METRICS_SIZE,
    ...
    def log_metrics(
        ...
        writer.add_histogram(
            'label_neg',
            metrics_t[METRICS_PRED_P_index, negLabel_mask],
            self.total_training_samples_count,
            bins=bins
        )
        writer.add_histogram(
            'label_pos',
            metrics_t[METRICS_PRED_P_index, posLabel_mask],
            self.total_training_samples_count,
            bins=bins
        )

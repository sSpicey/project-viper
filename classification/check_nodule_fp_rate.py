...
from classification.dsets import LunaDataset
from segmentation.dsets import Luna2dSegmentationDataset, get_Ct, get_candidates_info, get_candidate_info, CandidateInfo
import glob
...
class FalsePositiveRateCheck:
    ...
    def initialize_model_path(self, type_str):
        path = os.path.join(
            'data',
            'part2',
            'models',
    ...
    def initialize_models(self):
        ...
        self.convolutions = nn.ModuleList([
            self.make_convolution(radius).to(self.device) for radius in range(1, 8)
        ])
    ...
    def main(self):
        ...
        values = set(
            ...
            for candidate in values_dataset.candidate_info
        )
        ...
        for series_uid in series_iteration:
            ct, _, _, clean_g = self.segment_exam(series_uid)
            ...
            for candidate in candidate_info:
                ...
                for result_index, (result_center_Irc_t, nodule_probability_t) in enumerate(results):
                    result_center_xyz = Irc2xyz(result_center_Irc_t, ct.origin_xyz, ct.vx_size_xyz, ct.direction_a)
    ...
    def segment_exam(self, series_uid):
        with torch.no_grad():
            ct = get_Ct(series_uid)
            ...
            for batch_tuple in segmentation_dataloader:
                input_t, _, _, slice = batch_tuple

                input_g = input_t.to(self.device)
                prediction_g = self.segmentation_model(input_g)

                for i, slice in enumerate(slice):
                    output_g[slice] = prediction_g[i,0]
    ...
    def make_convolution(self, radius):
        diameter = 1 + radius * 2
    ...
    def segmentation_output(self, series_uid,  ct, clean_g):
        ...
        for i, center_Irc in enumerate(centerIrc_list):
            assert np.isfinite(center_Irc).all(), repr([series_uid, i, candidate_count, (ct.hu_a[candidate_label_a == i+1]).sum(), center_Irc])
            center_xyz = Irc2xyz(
                center_Irc,
                ct.origin_xyz,
                ct.vx_size_xyz,
            ...
            candidate = \
                CandidateInfo(None, None, None, diameter_mm, series_uid, center_xyz)
            candidate_info.append(candidate)

        return candidate_info, centerIrc_list, candidate_label_a

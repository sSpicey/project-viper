...
def get_candidate_info(required_on_disk=True):
    ...
    for candidate_info_tup in candidates_info:
        candidateInfo_dict.setdefault(candidate_info_tup.series_uid,
...
class Ct:
    def __init__(self, series_uid):
        ...
        self.positives_info = [
            candidate_tup
            for candidate_tup in candidates_info
            if candidate_tup.is_nodule
        ]
...
def get_Ct_raw_candidate(series_uid, center_xyz, width_Irc):
    ...
    ct_chunk, pos_chunk, center_Irc = ct.get_raw_candidate(center_xyz,
                                                         width_Irc)
...
class Luna2dSegmentationDataset(Dataset):
    def __init__(self,
        ...
        for series_uid in self.series_list:
            ...
            if self.fullCt_bool:
                self.sample_list += [(series_uid, slice_index)
        ...
        self.candidates_info = [cit for cit in self.candidates_info
                                   if cit.series_uid in series_set]

        self.positives = [nt for nt in self.candidates_info
                            if nt.is_nodule]

        log.info("{!r}: {} {} series, {} slices, {} nodules".format(
            self,
            len(self.series_list),
            {None: 'general', True: 'validation', False: 'training'}[is_value_set],
